...
def run_test_integers():
    ...
    # Test 2:
    expected = [4, 55, 22, 6, -3, 44, 7, -4, -3, -2, -1]
    answer = integers([(4, 55, 'banana'),
                       (22, 6, -3),
                       ['a', 'b', 44, '6', 7],
                       'goodbye',
                       [],
...
def run_test_big_letters():
    ...
    # Test 2
    expected = 'HOTEDYOKNWHAYSONEOBSQAPTHPBANANAACHEIOWY'
    answer = big_letters(['HellO ThEre',
                          'Do YOu KNoW tHe wAY?',
...
def big_letters(sequence_of_sequences):
    ...
    for k in range(len(sequence_of_sequences)):
        ...
        if type(sequence_of_sequences[k]) is str:
            for j in range(len(seq)):
                if type(seq[j]) is str and str.isupper(seq[j]):
                    new_string += str(seq[j])
    return new_string
